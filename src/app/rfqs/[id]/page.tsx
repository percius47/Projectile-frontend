...
export default function RfqDetailPage({
  ...
  const fetchRfqDetails = useCallback(async () => {
    try {
      ...
      // If RFQ is closed or awarded and user is a vendor, redirect to closed RFQ page
      if (
        user?.role === "vendor" &&
        (response.rfq.status === "awarded" || response.rfq.status === "closed")
      ) {
        router.push(`/rfqs/${rfqId}/closed`);
        return;
      }
  ...
  const handleSubmitQuote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const quoteData: CreateQuoteData = {
        rfq_id: rfqId,
        vendor_id: user.id, // Use user ID directly as vendor ID
